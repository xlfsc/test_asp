...
class Segment(AbstractData):
    ...
    def start_length_equ(self):
        print("length_equ rule start!")
        return 'test'

    def start_obj_equ(self):
        print("obj_equ rule start!")
        return 'test'
    ...
    def _():
        Segment.length_equ(X, Z) <= (Segment.length[X] == Y) & (Segment.length[Z] == Y) & (Y != None) & (
            K == (lambda X: X.start_length_equ()))
        # (Segment.obj_equ[X] == Y) <= (Z == (lambda X: X.start_obj_equ())) & (
...
class EqualRule(pyDatalog.Mixin):
    ...
    @pyDatalog.program()
    def _():
        EqualRule.obj_eq(X, Y) <= (Segment.point_A[X] == Segment.point_A[Y]) & (
...
class Triangle(AbstractData):
    ...
    def get_sum_of_inner_angle(self):
        ...
        return Express('{}+{}+{}'.format(self.angle_ABC.name, self.angle_ACB.name, self.angle_BAC.name))
    ...
    def _():
        ...
        #                                     & (U == (Y, Z)) & (U in (lambda o: Segment.length_equ(M, N).data)) & (
